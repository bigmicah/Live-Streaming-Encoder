...
export async function GET(
  ...
  try {
    ...
    if (!encodingSession) {
      return NextResponse.json({ error: 'Encoding session not found' }, { status: 404 })
    }
    return NextResponse.json(encodingSession)
  } catch (error) {
    console.error('Error fetching encoding session:', error)
    return NextResponse.json({ error: 'Failed to fetch encoding session' }, { status: 500 })
  }
}
...
export async function PUT(
  ...
  try {
    ...
    const { status, progress, inputBytes, outputBytes, pid, logPath, metadata } = body

    const existingSession = await db.encodingSession.findUnique({ where: { id: params.id } })
    if (!existingSession) {
      return NextResponse.json({ error: 'Encoding session not found' }, { status: 404 })
    }
    ...
    if (status === 'COMPLETED' || status === 'ERROR') updateData.endTime = new Date()
    ...
    if (status === 'COMPLETED' || status === 'ERROR' || status === 'STOPPING') {
      const newStreamStatus =
        status === 'COMPLETED' ? 'IDLE' : status === 'ERROR' ? 'ERROR' : 'STOPPING'
    ...
    return NextResponse.json({ error: 'Failed to update encoding session' }, { status: 500 })
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    await streamingEncoder.stopEncoding(params.id, existingSession.streamId)

    await db.encodingSession.delete({ where: { id: params.id } })
    ...
    return NextResponse.json({ error: 'Failed to delete encoding session' }, { status: 500 })
  }
}
